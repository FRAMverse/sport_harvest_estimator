...
<!-- If Focal Timestep is set to "Custom Strata", daily harvest will be calculated for this timespan. Otherwise, merely calculates WD and WE in span. -->
  ...
  if(input$focal_ts != -99){
    ...
    pssp |> 
      ...
      filter(area_code == input$focal_area,
             date_common_year >= as_date(gsub("^(.*?)-", "2020-", input$custom_start)),
             date_common_year <= as_date(gsub("^(.*?)-", "2020-", input$custom_end)),
             year %in% input$years) |> 
...
observeEvent(ignoreInit = TRUE,
               ...
               input$run_custom),
             {
               ...
               if(input$focal_ts == -99 & 
                  any(is.na(as_date(c(input$custom_start, input$custom_end))))){
                 showModal(
                   modalDialog(
                     title = 'Dates not workable',
                     'Custom strata dates are not readable. Use format YYYY-MM-DD'
                   )
                 )
...
### Custom Strata
```{r custom_strata}
textInput("custom_start", label = "Custom Strata Start Date",
          value = "2025-09-01", width = '100%')
textInput("custom_end", label = "Custom Strata End Date",
          value = "2025-09-30", width = '100%')
