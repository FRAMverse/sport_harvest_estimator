...
observeEvent(ignoreInit = TRUE,
  ...
  if(nrow(pssp_at()  %>% filter(regulation_type_code %in% input$msf)) == 0){
    ...
    mean_harvest <- pssp_at() |>
      ...
      summarize(total_retained_coho = sum(total_retained_coho), days = n(), .groups='drop') %>%
      mutate(coho_per_day = total_retained_coho / days) %>%
      group_by(time_step, area_code, day_type) %>%
      summarize(mean_daily_harvest = mean(coho_per_day, na.rm=T), .groups = 'drop') %>%
...
actionLink('select_recent_years', 'Select Recent 5 years')
...
renderPlot({
  ...
  plot %>%
    ...
    geom_col(alpha = 0.8) +
    labs(
      subtitle = paste("Area", input$focal_area, "during", paste(input$focal_ts, collapse = ","), 'RMSE: ', rmse, ' MAPE:', mape, 'MPE: ', mpe),
      x = NULL,
      y = 'Percent Error'
    ) +
...
renderPlot({
  mean_harvest_plot <- pssp_at() |>
    ...
    summarize(mean_est = mean(total_retained_coho, na.rm=T), .groups='drop')
