...
if (on_web_server){
  ...
  holidays <- read_csv(
    'https://www.federalpay.org/resources/holidays/Federal%20Holidays%20-%20all.csv'
  ) |>
    janitor::clean_names() |>
    mutate(date = lubridate::as_date(observed_date, format = '%B %d, %Y'),
           holiday = TRUE) |>
    select(date, holiday)
  ...
  save(list = c("pssp", "regs", "fs_ps_spt", "holidays"),
...
ts_lut = tribble(
  ...
  1, paste0(as.numeric(cur_year)+1, "-1-1"), paste0(as.numeric(cur_year)+1, "-6-30")
) |> 
...
days_spanned = reactive({
  if(all(!is.na(as_date(c(input$start_date, input$end_date))))){
    days.per.ts = tibble(
      ...
      left_join(holidays) |> # auguement monday + holday = WE
      mutate(
        day_type = if_else(lubridate::wday(date, label = T) == 'Mon' & !is.na(holiday), 'WE', day_type),
        year = lubridate::year(date),
...
observeEvent(
  ...
  {
    updateCheckboxGroupInput(inputId = 'years', selected = 2004:2024)
...
observeEvent(
  ...
  {
    updateCheckboxGroupInput(inputId = 'years', selected = 2018:2024)
  }
)
...
renderPlot({
  ...
  df_daily_means <- 
    tibble(day_type = c("WD", "WE"),
           cur_inputs = c(input$wday_df, input$wend_df))
  
  day_plot |> 
    ggplot(aes(factor(year), total_retained_coho, fill = day_type)) +
    geom_boxplot() +
    # geom_hline(data = df_daily_means,
    #            aes(yintercept = cur_inputs, col = day_type),
    #            linetype = 2) +
    labs(
      subtitle = paste("Area", input$focal_area, "during", paste(input$focal_ts, collapse = ",")),
