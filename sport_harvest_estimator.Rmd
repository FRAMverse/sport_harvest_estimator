...
radioButtons("msf", label = "Mark Selective",
             choiceValues = c('NS', 'MSF'), 
             choiceNames = c('No', 'Yes'), 
             selected = 'MSF', inline = T, width = '100%')
...
pssp_at <- reactive({
  filter(pssp, area_code == input$focal_area, time_step == input$focal_ts, year %in% input$years)
})

fram_at <- reactive({
  filter(fs_ps_spt, area_code == input$focal_area, ts == input$focal_ts, yr %in% input$years)
}) 

recent_regs_at <- reactive({
  filter(recent_regs, catch_area_code == input$focal_area, time_step == input$focal_ts)
})

pssp_reg_at <- reactive({
    filter(pssp, area_code == input$focal_area, time_step == input$focal_ts, year %in% input$years, regulation_type_code == input$msf)
})
...
observeEvent(ignoreInit = TRUE, 
    ...
    # catch errors
  if(nrow(pssp_reg_at()) == 0){
    modalDialog('There are no previous data to calculate mean harvest by day.
                Defaulting to whatever data available')
  } else {
      
    mean_harvest <- pssp_reg_at() |>
    rowwise() |>
      mutate(
...
numericInput("wday_hpd", label = "Weekday Harvest per angler per day",
...
numericInput("wend_hpd", label = "Weekend Harvest per angler per day",
            min = 0, max = 4, value = 0, step = 0.01,  width = '100%')
