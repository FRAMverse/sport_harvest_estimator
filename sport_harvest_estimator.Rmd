...
coho_regulations <- readxl::read_excel(
  path = "T:/DFW-Salmon Mgmt Modeling Team - General/Preseason/Coho/2022/sport_hvst_est_misc/coho_regulations.xlsx",
...
#substituting creel-based total caught from TG where possible, after 2015, mostly A5
...
rmis_cs <- readxl::read_excel("T:/DFW-Salmon Mgmt Modeling Team - General/Preseason/Coho/2022/SportSampleRateCoho2000_2020_creel_subs.xlsx", sheet = "creel_est_subs") |> 
  mutate(
    area_code = str_pad(str_squish(str_remove_all(catch_location_name, "MARINE SPORT AREA |MARINE SPORT PCA|\\.")), width = 2, pad = "0"),
    yr = factor(year),
    sr = if_else(is.na(sr_creel_sub), sr, sr_creel_sub)
  ) |> 
  select(FisheryID, area_code, year, yr, ts, caught, sampled, sr) |> 
  filter(
    !is.na(area_code),
    caught > 10, sampled > 10
    )
...
crc <- readRDS("T:/DFW-Salmon Mgmt Modeling Team - General/Catch datasets/CRC/d_spt_hvst_est_20220302.rds") |>
  filter(
...
gt::render_gt({
  ...
  pssp_at_gt <- pssp_at() |> 
    ...
    pivot_wider(names_from = wkend, values_from = c(n_obs, q50, mu, q95)) |> 
    arrange(desc(yr)) |>
    left_join(
